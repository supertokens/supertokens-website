        ...
        return __generator(this, function(_b) {
            switch (_b.label) {
                ...
                case 4:
                    ...
                    headers = new Headers();
                    if (!(preRequestLSS.status === "EXISTS")) return [3 /*break*/, 6];
                    return [4 /*yield*/, AntiCsrfToken.getToken(preRequestLSS.lastAccessTokenUpdate)];
                case 5:
                    antiCsrfToken = _b.sent();
                    if (antiCsrfToken !== undefined) {
                ...
                case 16:
                    ...
                    return [4 /*yield*/, getLocalSessionState(false)];
                case 17:
                    if (!(_b.sent().status === "NOT_EXISTS")) return [3 /*break*/, 20];
                    (0,
...
function fireSessionUpdateEventsIfNecessary(wasLoggedIn, status, frontTokenHeaderFromResponse) {
    // In case we've received a 401 that didn't clear the session (e.g.: we've sent no session token) then onUnauthorised will handle firing the UNAUTHORISED event
