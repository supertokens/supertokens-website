            ...
            switch (_a.label) {
                case 0:
                    if (preRequestIdToken === undefined) {
                        return [2 /*return*/, getIdRefreshToken() !== undefined];
                    }
                    return [
                        4 /*yield*/,
            ...
            return __generator(this, function(_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        preRequestIdToken = getIdRefreshToken();
                        return [
                            4 /*yield*/,
                            handleUnauthorised(
                        ...
                        return __generator(this, function(_a) {
                            switch (_a.label) {
                                ...
                                case 5:
                                    ...
                                    return [7 /*endfinally*/];
                                case 6:
                                    idCookieValue = getIdRefreshToken();
...
function getIdRefreshToken() {
    var fromLocalstorage = utils_1.getWindowOrThrow().localStorage.getItem(ID_REFRESH_TOKEN_NAME);
    if (fromLocalstorage !== null) {
        var splitted = fromLocalstorage.split(";");
        var value = splitted[0];
        var expires = Number(splitted[1]);
        if (expires < Date.now()) {
            setIdRefreshToken("remove");
            value = undefined;
        }
    ...
    var fromCookie = getIDFromCookieOld();
...
function setIdRefreshToken(idRefreshToken) {
    if (idRefreshToken === "remove") {
        utils_1.getWindowOrThrow().localStorage.removeItem(ID_REFRESH_TOKEN_NAME);
    } else {
        utils_1.getWindowOrThrow().localStorage.setItem(ID_REFRESH_TOKEN_NAME, idRefreshToken);
    }
    // for backwards compatibility
...
function getAntiCSRFToken() {
    var fromLocalstorage = utils_1.getWindowOrThrow().localStorage.getItem(ANTI_CSRF_NAME);
    if (fromLocalstorage !== null) {
    ...
    var fromCookie = getAntiCSRFromCookieOld();
    if (fromCookie !== null) {
...
function setAntiCSRF(antiCSRFToken) {
    if (antiCSRFToken === undefined) {
        utils_1.getWindowOrThrow().localStorage.removeItem(ANTI_CSRF_NAME);
    } else {
        utils_1.getWindowOrThrow().localStorage.setItem(ANTI_CSRF_NAME, antiCSRFToken);
    }
    // for backwards compatibility
...
function getFrontToken() {
    var fromLocalstorage = utils_1.getWindowOrThrow().localStorage.getItem(FRONT_TOKEN_NAME);
    if (fromLocalstorage !== null) {
        return fromLocalstorage;
    }
    // for backwards compatibility
    ...
    var fromCookie = getFrontTokenFromCookie();
    if (fromCookie !== null) {
...
function setFrontToken(frontToken) {
    if (frontToken === undefined) {
        utils_1.getWindowOrThrow().localStorage.removeItem(FRONT_TOKEN_NAME);
    } else {
        utils_1.getWindowOrThrow().localStorage.setItem(FRONT_TOKEN_NAME, frontToken);
    }
