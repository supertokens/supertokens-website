...
function shouldDoInterceptionBasedOnUrl(toCheckUrl, apiDomain, sessionDomain) {
    ...
    if (sessionDomain === undefined) {
        ...
        var normalisedsessionDomain = normaliseSessionScopeOrThrowError(sessionDomain);
        if (sessionDomain.split(":").length > 1) {
            // means port may provided
            var portStr = sessionDomain.split(":")[sessionDomain.split(":").length - 1];
