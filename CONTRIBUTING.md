...
## Testing
1. Navigate to the `supertokens-website` repository
2. Start the core
   ```
   docker compose up --wait
   ```
3. Start a node server required for testing
   ```
   cd ./test/server/
   npm i -d
   ...
   ```
4. Open a new terminal in `supertokens-website` and run all tests
   ```
